  ...
  const overallStats =
    latestSummary && earliestSummary
      ? {
          ...
          highRiskChange:
            latestSummary.highRiskCount - earliestSummary.highRiskCount,
          highRiskChangePct: (
            ((latestSummary.highRiskCount - earliestSummary.highRiskCount) /
              earliestSummary.highRiskCount) *
            100
          ).toFixed(1),
          avgHighRiskPct: (
            summaries.reduce(
              (acc, s) => acc + (s.highRiskCount / s.evaluated) * 100,
              0,
            ) / summaries.length
          ).toFixed(1),
...
function printTable(
  data: Awaited<ReturnType<typeof queryTrends>>,
  options: QueryOptions,
) {
  const { trendData, topMovers, newHighRisk, promotedStocks, overallStats } =
    data;
  ...
  if (overallStats) {
    ...
    console.log(
      `   High Risk Change: ${overallStats.earliestHighRisk} → ${overallStats.latestHighRisk} (${overallStats.highRiskChange > 0 ? "+" : ""}${overallStats.highRiskChange}, ${overallStats.highRiskChangePct}%)`,
    );
    console.log(`   Avg High Risk %: ${overallStats.avgHighRiskPct}%`);
    console.log(
      `   High Risk Range: ${overallStats.minHighRisk} - ${overallStats.maxHighRisk}`,
    );
  }
  ...
  if (trendData.length > 0) {
    ...
    console.log("─".repeat(100));
    console.log(
      "Date       | Total  | LOW    | MEDIUM | HIGH   | HIGH%  | P&D  | Pump | Vol",
    );
    console.log("─".repeat(100));
  ...
  if (topMovers.length > 0) {
    ...
    for (const mover of topMovers) {
      ...
      console.log(
        `   ${mover.symbol}: ${mover.fromRiskLevel} → ${mover.toRiskLevel} (${direction}${Math.abs(mover.scoreChange)} pts)`,
      );
    }
  }
  ...
  if (newHighRisk.length > 0) {
    ...
    for (const alert of newHighRisk) {
      ...
      console.log(
        `            Score: ${alert.newScore}, Price: $${alert.priceAtAlert?.toFixed(2) || "N/A"}`,
      );
    }
  }
  ...
  if (promotedStocks.length > 0) {
    ...
    for (const stock of promotedStocks) {
      console.log(
        `   ${stock.symbol}: ${stock.promotionCount} mentions (${stock.name})`,
      );
    }
  }
  ...
  if (trendData.length > 5) {
    ...
    console.log(
      "       " +
        trendData[0].date.slice(5) +
        " ".repeat(Math.max(0, highRiskValues.length - 12)) +
        trendData[trendData.length - 1].date.slice(5),
    );
  }
