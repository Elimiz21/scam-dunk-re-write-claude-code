...
function parseArgs(): QueryOptions {
  ...
  if (args.length === 0 || args[0].startsWith("-")) {
    console.error(
      "Usage: npm run query:history -- SYMBOL [--days N] [--format table|json]",
    );
    process.exit(1);
  }
...
function printTable(data: Awaited<ReturnType<typeof getStockHistory>>) {
  ...
  if (snapshots.length > 0) {
    ...
    console.log(
      `   Market Cap: $${latest.marketCap?.toLocaleString() || "N/A"}`,
    );
  ...
  console.log("─".repeat(80));
  console.log(
    "Date        | Risk   | Score | Price      | Market Cap      | Signals",
  );
  console.log("─".repeat(80));
  ...
  for (const snap of snapshots) {
    ...
    const price = snap.lastPrice
      ? `$${snap.lastPrice.toFixed(2)}`.padStart(10)
      : "N/A".padStart(10);
    ...
    console.log(
      `${date} | ${risk} | ${score} | ${price} | ${marketCap} | ${signals}`,
    );
  }
  ...
  if (riskChanges.length > 0) {
    ...
    for (const change of riskChanges.slice(0, 5)) {
      ...
      console.log(
        `   ${from} → ${to}: ${change.fromRiskLevel} → ${change.toRiskLevel} (${direction}${Math.abs(change.scoreChange)} pts)`,
      );
      if (change.priceChangePct) {
        console.log(
          `      Price change: ${change.priceChangePct > 0 ? "+" : ""}${change.priceChangePct.toFixed(1)}%`,
        );
      }
    }
  }
  ...
  if (socialScans.length > 0) {
    ...
    for (const scan of socialScans.slice(0, 5)) {
      ...
      console.log(
        `   ${date}: ${scan.promotionSource || "Unknown"} - ${scan.promoterName || "Unknown"}`,
      );
  ...
  if (alerts.length > 0) {
    ...
    for (const alert of alerts.slice(0, 5)) {
      ...
      console.log(
        `   ${date}: ${alert.alertType} - ${alert.previousRiskLevel || "N/A"} → ${alert.newRiskLevel}`,
      );
    }
  }
