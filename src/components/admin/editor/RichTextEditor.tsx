...
export default function RichTextEditor({
  ...
  const editor = useEditor({
    extensions: [
      ...
      TableHeader.configure({
        HTMLAttributes: {
          class:
            "border border-border px-3 py-2 text-sm font-semibold bg-secondary/50",
        },
      }),
    ...
    editorProps: {
      attributes: {
        class:
          "prose prose-sm sm:prose-base max-w-none focus:outline-none min-h-[400px] px-5 py-4",
      },
  ...
  return (
    <div
      ...
      className="relative border border-border rounded-2xl bg-card overflow-hidden"
    >
      {/* Main Toolbar */}
      <EditorToolbar
        editor={editor}
      ...
      <div className="flex items-center justify-between px-4 py-2 border-t border-border bg-secondary/20 text-xs text-muted-foreground">
        ...
        <span className="text-xs">
          Type{" "}
          <kbd className="px-1 py-0.5 bg-secondary rounded text-[10px] font-mono">
            /
          </kbd>{" "}
          for commands
        </span>
