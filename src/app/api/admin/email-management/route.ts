...
async function safeEmailLogQuery<T>(
  fn: () => Promise<T>,
  fallback: T,
): Promise<T> {
  try {
    ...
    if (!prisma.emailLog) {
      console.warn(
        "[EMAIL MGMT] emailLog model not available on Prisma client - regenerate client",
      );
      return fallback;
    }
    return await fn();
    ...
    console.warn(
      "[EMAIL MGMT] emailLog query failed (table may not exist yet):",
      message,
    );
    return fallback;
  }
  ...
  try {
    ...
    if (section === "logs") {
      ...
      const [logs, total] = await Promise.all([
        ...
        safeEmailLogQuery(() => prisma.emailLog.count({ where }), 0),
      ]);
...
async function getEmailLogStats() {
  ...
  return safeEmailLogQuery(async () => {
    ...
    const [totalSent, totalFailed, sent24h, failed24h, sent7d, failed7d] =
      await Promise.all([
        prisma.emailLog.count({ where: { status: "SENT" } }),
        prisma.emailLog.count({ where: { status: "FAILED" } }),
        prisma.emailLog.count({
          where: { status: "SENT", createdAt: { gte: last24h } },
        }),
        prisma.emailLog.count({
          where: { status: "FAILED", createdAt: { gte: last24h } },
        }),
        prisma.emailLog.count({
          where: { status: "SENT", createdAt: { gte: last7d } },
        }),
        prisma.emailLog.count({
