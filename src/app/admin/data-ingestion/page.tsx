...
export default function DataIngestionPage() {
  ...
  async function runBackfill() {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to backfill metrics",
      );
    } finally {
  ...
  return (
    <AdminLayout>
      <div className="space-y-6">
        ...
        {dbStatus && (
          <div
            className={`bg-card rounded-2xl shadow p-6 border-l-4 ${
            ...
            <div className="mt-4 grid grid-cols-2 md:grid-cols-5 gap-2">
              {Object.entries(dbStatus.tables).map(([name, info]) => (
                <div key={name} className="flex items-center gap-2 text-sm">
                  ...
                  <span
                    className={info.exists ? "text-foreground" : "text-red-600"}
                  >
                    {name}
              ...
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  ...
                  <p className="text-2xl font-bold">
                    {status?.availableDates.length || 0}
                  </p>
                </div>
              </div>
            </div>
                ...
                <div className="mt-3 text-sm text-foreground space-y-1">
                  <div>
                    ...
                    <span
                      className={`font-medium ${
                        lastIngestionDetails?.status === "SUCCESS"
                          ? "text-green-600"
                          : "text-red-600"
                      }`}
                    >
                  ...
                  {lastIngestionDetails?.error && (
                    <div className="text-red-600">
                      Error:{" "}
                      {lastIngestionDetails.errorType
                        ? `${lastIngestionDetails.errorType} - `
                        : ""}
                      {lastIngestionDetails.error}
                    </div>
                  ...
                  {lastIngestionDetails?.durationMs !== undefined && (
                    <div>
                      Duration: {Math.round(lastIngestionDetails.durationMs)}ms
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="bg-card rounded-2xl shadow p-6">
                ...
                <div className="mt-3 text-sm text-foreground space-y-1">
                  <div>
                    Total Scan History:{" "}
                    <span className="font-medium">
                      {backfillResult.totalScanHistory}
                    </span>
                  </div>
                  <div>
                    Model Metrics Days:{" "}
                    <span className="font-medium">
                      {backfillResult.modelMetricsDays}
                    </span>
                  </div>
                  <div>
                    Scan Usage Entries:{" "}
                    <span className="font-medium">
                      {backfillResult.scanUsageEntries}
                    </span>
                  </div>
                </div>
              </div>
        ...
        <div className="bg-card rounded-2xl shadow p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Rebuild ModelMetrics and ScanUsage from ScanHistory for live
                data consistency.
              </p>
            </div>
            <button
          ...
          <div className="bg-card rounded-2xl shadow">
            ...
            <div className="divide-y divide-border">
              {status.pendingDates
                ...
                .reverse()
                .map((date) => {
                  const fileInfo = status.fileStatus?.find(
                    (f) => f.date === date,
                  );
                  return (
                    <div
          ...
          <div className="bg-card rounded-2xl shadow">
            <div className="px-6 py-4 border-b border-border">
              ...
              <p className="text-sm text-muted-foreground mt-1">
                Click on a date to re-import if needed
              </p>
            </div>
                ...
                <div key={idx} className="px-6 py-4">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-sm text-muted-foreground">
                    ...
                    <div>
                      ...
                      {result.promotedStocksCreated || 0}
                    </div>
                  </div>
                </div>
              ))}
        ...
        {status?.debug && (
          <div className="bg-secondary/50 rounded-2xl shadow p-4 text-sm">
            ...
            <p className="text-muted-foreground">
              Total files in bucket: {status.debug.filesFound}
            </p>
            <p className="text-muted-foreground">
              Evaluation files: {status.debug.evaluationFiles} (enhanced:{" "}
              {status.debug.enhancedEvaluationFiles}, legacy:{" "}
              {status.debug.legacyEvaluationFiles})
            </p>
            <p className="text-muted-foreground">
              Summary files: {status.debug.summaryFiles}
            </p>
            <p className="text-muted-foreground">
              Promoted stocks files: {status.debug.promotedFiles}
            </p>
            <p className="text-muted-foreground">
              Comparison files: {status.debug.comparisonFiles}
            </p>
            {status.debug.allFileNames.length > 0 ? (
                  ...
                  <li
                    ...
                    className={`font-mono text-xs ${
                      name.startsWith("enhanced-evaluation-")
                        ? "text-green-600"
                        : name.startsWith("fmp-evaluation-")
                          ? "text-green-600"
                          : name.startsWith("fmp-summary-")
                            ? "text-blue-600"
                            : name.startsWith("promoted-stocks-")
                              ? "text-purple-600"
                              : name.startsWith("comparison-")
                                ? "text-orange-600"
                                : name.startsWith("enhanced-high-risk-")
                                  ? "text-red-600"
                                  : name.startsWith("suspicious-stocks-")
                                    ? "text-red-600"
                                    : name.startsWith("daily-report-")
                                      ? "text-blue-600"
                                      : name.startsWith("social-media-scan-")
                                        ? "text-cyan-600"
                                        : "text-muted-foreground"
                    }`}
                  >
              ...
              <p className="text-orange-600 mt-2">
                No files found. Either the bucket is empty or RLS policy is
                missing. Run: CREATE POLICY &quot;Allow public read access to
                evaluation-data&quot; ON storage.objects FOR SELECT USING
                (bucket_id = &apos;evaluation-data&apos;);
              </p>
            )}
          </div>
        )}
          ...
          status?.ingestedDates.length === 0 && (
            <div className="bg-card rounded-2xl shadow p-12 text-center">
              ...
              <p className="mt-2 text-muted-foreground">
                Upload evaluation files to Supabase Storage bucket
                &apos;evaluation-data&apos;.
              </p>
            </div>
          )}
