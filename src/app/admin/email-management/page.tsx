...
export default function EmailManagementPage() {
  ...
  async function toggleRecipientActive(recipient: Recipient) {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to toggle recipient",
      );
    } finally {
      setRecipientActionLoading(false);
    }
  ...
  async function handleSendTestEmail() {
    ...
    try {
      ...
      if (data.success) {
        setSuccess(
          `Test email sent to ${testEmail}${data.resendId ? ` (ID: ${data.resendId})` : ""}`,
        );
      } else {
        ...
        <div className="flex justify-between items-start">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Configure email routing, manage recipients, send emails, and
              monitor delivery
            </p>
          </div>
          <button
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-amber-800 mt-1">
                  Resend is using the test domain (
                  <code className="bg-amber-100 px-1 rounded">
                    onboarding@resend.dev
                  </code>
                  ). Emails can only be sent to the Resend account owner&apos;s
                  email. To send to all users, verify a custom domain at{" "}
                  <a
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  <div className="flex items-center gap-3">
                    <div
                      className={`h-3 w-3 rounded-full ${config?.isValid ? "bg-green-500" : "bg-red-500"}`}
                    />
                    <span className="font-medium">
                      {config?.isValid
                        ? "Email service configured"
                        : "Email service has errors"}
                    </span>
                  </div>
              ...
              {config?.warnings && config.warnings.length > 0 && (
                <div className="space-y-2">
                  ...
                  {config.warnings.map((w, i) => (
                    <div
                      key={i}
                      className="flex items-start gap-2 text-sm text-amber-700 bg-amber-50 p-3 rounded"
                    >
              ...
              {config?.errors && config.errors.length > 0 && (
                <div className="space-y-2">
                  ...
                  {config.errors.map((e, i) => (
                    <div
                      key={i}
                      className="flex items-start gap-2 text-sm text-red-700 bg-red-50 p-3 rounded"
                    >
              ...
              <div className="border-t pt-6">
                ...
                <div className="bg-secondary/50 rounded-2xl p-4 text-sm text-muted-foreground space-y-2">
                  <p>
                    When a support ticket is submitted via the Contact Us form:
                  </p>
                  <ol className="list-decimal list-inside space-y-1 ml-2">
                    <li>
                      <strong>Primary recipients</strong> always receive
                      notifications for all ticket categories
                    </li>
                    <li>
                      <strong>Category-specific recipients</strong> only receive
                      notifications for their assigned categories
                    </li>
                    <li>
                      <strong>Recipients with no categories set</strong> receive
                      all ticket notifications
                    </li>
                    <li>
                      If no recipients are configured, emails fall back to the
                      default admin email
                    </li>
                    <li>
                      The user who submitted the ticket receives a confirmation
                      email
                    </li>
                  </ol>
                </div>
              ...
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    Manage who receives support ticket notifications and which
                  ...
                  {!addForm.isPrimary && (
                    <div>
                      <label className="block text-sm font-medium text-foreground mb-2">
                        Category Routing (leave empty for all)
                      </label>
              ...
              {recipients.length === 0 ? (
                <div className="text-center py-12 text-muted-foreground">
                  ...
                  <p className="text-sm mt-1">
                    Add recipients to start routing support ticket notifications
                  </p>
                </div>
              ) : (
          ...
          {activeTab === "compose" && (
            <div className="p-6 space-y-6">
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  Send an email directly from the admin dashboard using your
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    The message will be wrapped in a branded ScamDunk email
                    template.
                  </p>
                </div>
                <button
          ...
          {activeTab === "logs" && (
            <div className="p-6 space-y-6">
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  Track all emails sent from the system and monitor delivery
                  status
                </p>
              </div>
                        ...
                        {logs.map((log) => (
                          <tr key={log.id} className="hover:bg-secondary">
                            <td className="px-4 py-3 whitespace-nowrap">
                              <span
                                className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium ${STATUS_STYLES[log.status] || "bg-secondary text-foreground"}`}
                              >
                                {log.status === "SENT" && (
                            ...
                            <td className="px-4 py-3 whitespace-nowrap">
                              {log.errorMessage ? (
                                <span
                                  className="text-xs text-red-600 truncate block max-w-xs"
                  ...
                  {logPagination.pages > 1 && (
                    <div className="flex items-center justify-between border-t pt-4">
                      <p className="text-sm text-muted-foreground">
                        ...
                        {Math.min(logPagination.page * 25, logPagination.total)}{" "}
                        of {logPagination.total}
                      </p>
                      <div className="flex gap-2">
                        <button
                          onClick={() =>
            ...
            <div className="p-6 space-y-6">
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  ...
                  to automatically create support tickets when users email
                  directly
                </p>
              </div>
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-2xl p-5">
                ...
                <p className="text-sm text-blue-800 mb-3">
                  When someone emails <strong>support@scamdunk.com</strong>, the
                  email gets forwarded to your webhook endpoint. The system
                  automatically:
                </p>
                <ol className="list-decimal list-inside text-sm text-blue-800 space-y-1 ml-2">
                  ...
                  <li>
                    Auto-detects the category (billing, bug, feedback, etc.)
                  </li>
                  <li>
                    Routes notifications to the correct recipients based on
                    category
                  </li>
                  <li>Sends a confirmation email back to the sender</li>
                  <li>
                    The ticket appears in Customer Support for your team to
                    manage
                  </li>
                </ol>
              </div>
              ...
              <div className="bg-secondary/50 rounded-2xl p-5">
                ...
                <p className="text-xs text-muted-foreground mt-2">
                  Use this URL in your email forwarding service or Resend
              ...
              <div className="space-y-4">
                ...
                <div className="border border-border rounded-2xl p-5">
                  ...
                  <ol className="list-decimal list-inside text-sm text-muted-foreground space-y-2 ml-2">
                    ...
                    <li>
                      Add the required MX record to your DNS to route inbound
                      mail through Resend
                    </li>
                    <li>
                      In Resend, configure an inbound webhook pointing to the
                      URL above
                    </li>
                    <li>
                      Set{" "}
                      ...
                      in your Vercel environment variables for security
                    </li>
                    <li>
                      Emails to{" "}
                      <code className="bg-secondary px-1 rounded">
                ...
                <div className="border border-border rounded-2xl p-5">
                  ...
                  <ol className="list-decimal list-inside text-sm text-muted-foreground space-y-2 ml-2">
                    <li>
                      ...
                      with any email provider (Google Workspace, Zoho, etc.)
                    </li>
                    <li>
                      Use a service like Zapier, Make, or Pipedream to forward
                      incoming emails as JSON to the webhook URL
                    </li>
  ...
  "html": "<p>Optional HTML body</p>"
                ...
                <div className="border border-amber-200 bg-amber-50 rounded-2xl p-5">
                  ...
                  <p className="text-sm text-amber-800">
                    ...
                    in your Vercel environment variables to prevent unauthorized
                    ticket creation. The webhook will verify this secret via the{" "}
                    <code className="bg-amber-100 px-1 rounded">
