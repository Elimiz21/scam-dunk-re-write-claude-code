...
export default function AdminBlogEditorPage() {
  ...
  async function processFile(file: File) {
    ...
    const isTextFormat =
      textFormats.includes(fileExt) || file.type.startsWith("text/");
    const isHtmlFormat =
      htmlFormats.includes(fileExt) || file.type === "text/html";
    const isDocxFormat =
      docFormats.includes(fileExt) ||
      file.type ===
        "application/vnd.openxmlformats-officedocument.wordprocessingml.document";

    if (!isTextFormat && !isHtmlFormat && !isDocxFormat) {
      alert(
        `Unsupported file format (.${fileExt}). Supported formats: ${allFormats.join(", ")}`,
      );
      return;
    }
      ...
      <div className="space-y-6">
          ...
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                {isNew
                  ? "Create a new blog post with the rich editor"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div
              className={`relative rounded-2xl shadow transition-all duration-200 ${
                isDragging
                ...
                <div className="flex items-start gap-4">
                  <div
                    className={`flex-shrink-0 p-3 rounded-xl transition-colors ${
                      isDragging ? "bg-primary/15" : "bg-card/50"
                    }`}
                  >
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                    post.isPublished ? "gradient-brand" : "bg-secondary"
                  }`}
                >
                  <span
