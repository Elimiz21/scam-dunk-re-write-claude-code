...
export default function StockDeepDivePage() {
    ...
    return (
      <AdminLayout>
        ...
        <AlertBanner
          ...
          message={error || "No data available for this stock."}
        />
      </AdminLayout>
    );
  }
  ...
  const socialMentions =
    socialData?.socialMediaMentions ??
    stock.socialMediaFindings?.socialMediaMentions ??
    [];
  const promotionScore =
    socialData?.overallPromotionScore ??
    stock.socialMediaFindings?.overallPromotionScore ??
    0;
      ...
      <div className="space-y-5">
        ...
        >
          ...
          <div className="flex items-center gap-2 mt-3 text-xs text-muted-foreground">
            ...
            Scan date: {data.foundDate} at{" "}
            {new Date(stock.evaluatedAt).toLocaleTimeString()}
          </div>
        </div>
              ...
              <div className="space-y-2">
                {newsItems.map((news, i) => (
                  ...
                  >
                    ...
                    <div>
                      <p className="text-xs text-foreground group-hover:text-primary line-clamp-2">
                        {news.title}
                      </p>
        ...
        {socialMentions.length > 0 && (
          <div className="bg-card rounded-2xl border border-border p-5">
                  ...
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span
                      className={`text-[9px] px-1.5 py-0.5 rounded font-medium ${
                        m.promotionRisk === "high"
                          ? "bg-red-500/10 text-red-600"
                          : m.promotionRisk === "medium"
                            ? "bg-amber-500/10 text-amber-600"
            ...
            {(socialData?.overallAssessment ||
              stock.socialMediaFindings?.overallAssessment) && (
          ...
          <div className="bg-card rounded-2xl border border-red-500/20 p-5">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span
                className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${
                  schemeData.status === "ONGOING"
                    ? "bg-red-500/10 text-red-600"
                    : schemeData.status === "COOLING"
                      ? "bg-amber-500/10 text-amber-600"
                      : "bg-emerald-500/10 text-emerald-600"
                }`}
              >
            ...
            <div className="grid grid-cols-3 gap-4 mb-4">
              <div className="rounded-lg bg-secondary/50 p-3">
                ...
                <div className="text-lg font-bold tabular-nums">
                  ${schemeData.priceAtDetection.toFixed(2)}
                </div>
                <div className="text-[10px] text-muted-foreground">
                  {schemeData.firstDetected}
                </div>
              </div>
              <div className="rounded-lg bg-secondary/50 p-3">
                <div className="text-[10px] text-muted-foreground mb-1">
                ...
                <div className="text-lg font-bold tabular-nums">
                  ${schemeData.peakPrice.toFixed(2)}
                </div>
              </div>
              <div className="rounded-lg bg-secondary/50 p-3">
                <div className="text-[10px] text-muted-foreground mb-1">
                ...
                <div className="flex items-center gap-1">
                  <span className="text-lg font-bold tabular-nums">
                    ${schemeData.currentPrice.toFixed(2)}
                  </span>
                  {schemeData.priceChangeFromPeak < -5 && (
                ...
                <div
                  className={`text-xs font-bold tabular-nums ${schemeData.priceChangeFromPeak < 0 ? "text-red-500" : "text-emerald-500"}`}
                >
                  {schemeData.priceChangeFromPeak > 0 ? "+" : ""}
                  {schemeData.priceChangeFromPeak.toFixed(1)}% from peak
                </div>
              </div>
            </div>
                    ...
                    <div key={i} className="flex items-start gap-3 relative">
                      ...
                      <div className="flex-1 pb-2">
                        <div className="flex items-center gap-2">
                          ...
                          <span
                            className={`text-xs ${evt.significance === "high" ? "font-medium text-foreground" : "text-muted-foreground"}`}
                          >
                            {evt.event}
                          </span>
                        </div>
            ...
            {schemeData.promotionPlatforms.length > 0 && (
              <div className="mt-3 pt-3 border-t border-border">
                ...
                {schemeData.promotionPlatforms.map((p) => (
                  <span
                    key={p}
                    className="text-xs px-1.5 py-0.5 rounded bg-secondary text-muted-foreground mx-0.5"
                  >
                    {p}
                  </span>
                ))}
