...
const MOCK_HIGH_RISK: RiskResponse = {
  ...
  signals: [
    {
      code: "MICROCAP_PRICE",
      category: "STRUCTURAL",
      description: "Penny stock under $5",
      weight: 15,
    },
    {
      code: "VOLUME_EXPLOSION",
      category: "PATTERN",
      description: "Volume 8x normal average",
      weight: 20,
    },
    {
      code: "SPIKE_7D",
      category: "PATTERN",
      description: "Price up 340% in 7 days",
      weight: 25,
    },
    {
      code: "UNSOLICITED",
  ...
  narrative: {
    header:
      "Multiple serious red flags detected. This stock shows classic pump-and-dump characteristics including a massive price spike, exploding volume, and penny stock structure traded on OTC markets.",
    stockRedFlags: [
...
const MOCK_MEDIUM_RISK: RiskResponse = {
  ...
  signals: [
    {
      ...
      category: "STRUCTURAL",
      description: "Market cap under $300M",
      weight: 8,
    },
    {
      code: "SPIKE_7D",
      category: "PATTERN",
      description: "Price up 28% in 7 days",
      weight: 12,
    },
    {
      code: "OVERBOUGHT_RSI",
  ...
  narrative: {
    header:
      "Some warning signs detected. While this stock trades on a major exchange, recent price activity and momentum indicators suggest caution is warranted before investing.",
    stockRedFlags: [
...
const MOCK_LOW_RISK: RiskResponse = {
  ...
  narrative: {
    header:
      "This is a well-established, large-cap company trading on a major exchange with high liquidity. Our scan found very few red flags associated with common investment scams.",
    stockRedFlags: [
...
const STEPS_MID_PROGRESS: Step[] = [
  {
    label: "Validating ticker symbol",
    status: "complete",
    detail: "XYZQ is valid",
  },
  {
...
const STEPS_NEAR_DONE: Step[] = [
  ...
  {
    label: "Checking regulatory alerts",
    status: "complete",
...
const TAB_DESCRIPTIONS: Record<
  string,
  { color: string; dotColor: string; bg: string; title: string; desc: string }
> = {
  "results-high": {
    color: "text-red-600",
    dotColor: "bg-red-500",
...
export default function DesignPreviewPage() {
  ...
  const resultForTab: Record<
    string,
    { result: RiskResponse; hasChatData: boolean }
  > = {
  ...
  return (
    <div
      className={`bg-background ${isResultTab ? "h-screen overflow-hidden flex flex-col" : "min-h-screen"}`}
    >
      {/* Header */}
      ...
      {isResultTab && tabMeta && (
        <>
          ...
          <div
            className={`flex-shrink-0 px-4 py-2 border-b border-border/50 ${tabMeta.bg}`}
          >
            <h2 className="text-sm font-bold flex items-center gap-2">
              <span
                className={`h-2.5 w-2.5 rounded-full ${tabMeta.dotColor}`}
              />
              {tabMeta.title}
            </h2>
        ...
        <div className="animate-fade-in p-4 space-y-8 max-w-7xl mx-auto">
          <div className="px-4 py-3 border-b border-border/50">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              All steps visible from the start in fuzzy/dim text. They animate
              to green as completed. Tagline headline + subtext shown together,
              rotating every 4 seconds. Minimum 8-second scan.
            </p>
          </div>
              ...
              <div className="p-4 rounded-xl border border-border bg-card">
                ...
                <LoadingStepper
                  ...
                  currentTip={{
                    ...
                    subtext:
                      "It's called doing your homework. We made it easy.",
                  }}
                />
              </div>
            </div>
          ...
          <div className="p-6 rounded-xl border border-primary/20 bg-primary/5">
            ...
            <ul className="space-y-2 text-sm text-muted-foreground">
              <li className="flex items-start gap-2">
                <span className="h-5 w-5 rounded-full bg-emerald-500 flex items-center justify-center flex-shrink-0 mt-0.5 text-white text-[10px] font-bold">
                  1
                </span>
                <span>
                  <strong>8-second minimum scan</strong> — Results held until at
                  least 8 seconds have passed.
                </span>
              </li>
              <li className="flex items-start gap-2">
                <span className="h-5 w-5 rounded-full bg-emerald-500 flex items-center justify-center flex-shrink-0 mt-0.5 text-white text-[10px] font-bold">
                ...
                <span>
                  <strong>Split-screen layout</strong> — Left 60%: scorecard
                  (scrollable). Right 40%: info panels. No page scroll.
                  Disclaimer bar pinned at bottom.
                </span>
              </li>
              <li className="flex items-start gap-2">
                <span className="h-5 w-5 rounded-full bg-emerald-500 flex items-center justify-center flex-shrink-0 mt-0.5 text-white text-[10px] font-bold">
                ...
                <span>
                  <strong>Info panels with (i) hover</strong> — How it Works,
                  What We Check, After the Test, Disclaimer & Terms.
                </span>
              </li>
              <li className="flex items-start gap-2">
                <span className="h-5 w-5 rounded-full bg-emerald-500 flex items-center justify-center flex-shrink-0 mt-0.5 text-white text-[10px] font-bold">
                ...
                <span>
                  <strong>Tagline pairs (scan only)</strong> — Headline +
                  subtext shown together during scan, rotating every 4 seconds.
                  Not shown on results page.
                </span>
              </li>
              <li className="flex items-start gap-2">
                <span className="h-5 w-5 rounded-full bg-emerald-500 flex items-center justify-center flex-shrink-0 mt-0.5 text-white text-[10px] font-bold">
                ...
                <span>
                  <strong>Fuzzy-to-green steps</strong> — All steps visible
                  upfront in dim/blurred text, animate to green when done.
                </span>
              </li>
              <li className="flex items-start gap-2">
                <span className="h-5 w-5 rounded-full bg-orange-500 flex items-center justify-center flex-shrink-0 mt-0.5 text-white text-[10px] font-bold">
                  6
                </span>
                <span>
                  <strong>Orangey-red missing data notices</strong> — When no
                  pitch text or chat is uploaded, those sections show prominent
                  orange-red warnings saying &quot;Not analyzed.&quot;
                </span>
              </li>
              <li className="flex items-start gap-2">
                <span className="h-5 w-5 rounded-full bg-emerald-500 flex items-center justify-center flex-shrink-0 mt-0.5 text-white text-[10px] font-bold">
                ...
                <span>
                  <strong>Risk-colored borders & ticker</strong> — Scorecard
                  border and ticker text match risk level: green/orange/red.
                </span>
              </li>
            </ul>
          </div>
