...
async function testEnhancedPipeline(): Promise<void> {
  ...
  if (files.length === 0) {
    ...
    console.log(
      "Please run the FMP evaluation first, or set FMP_API_KEY for a full test.",
    );
    process.exit(1);
  }
  ...
  for (const stock of highRiskStocks) {
    ...
    if (
      !filtered &&
      stock.avgDollarVolume &&
      stock.avgDollarVolume > VOLUME_THRESHOLD
    ) {
      filtered = true;
  ...
  for (let i = 0; i < testStocks.length; i++) {
    ...
    try {
      ...
      console.log(
        `  Aggregated Promotion Score: ${socialScan.aggregatedScore}/100`,
      );
      console.log(
        `  Risk Assessment: ${socialScan.riskAssessment.overallPromotionRisk.toUpperCase()}`,
      );
      console.log(
        `  Urgency: ${socialScan.riskAssessment.urgency.toUpperCase()}`,
      );

      if (socialScan.keyFindings.length > 0) {
        console.log(
          `  Key Findings: ${socialScan.keyFindings.slice(0, 3).join(", ")}`,
        );
      }
    } catch (error: any) {
  ...
  const testResultsPath = path.join(
    RESULTS_DIR,
    `test-enhanced-results-${today}.json`,
  );
  fs.writeFileSync(
    testResultsPath,
    JSON.stringify(
      {
        ...
        results: scanResults.map((r) => ({
          ...
          platforms:
            r.socialScan?.platformResults.map((p) => ({
              name: p.platform,
  ...
  if (highPromotionStocks.length === 0) {
    ...
    for (const result of highPromotionStocks) {
      ...
      console.log(
        `    Key Findings: ${result.socialScan.keyFindings.join(", ")}`,
      );
    }
  }
  ...
  if (summaries.length > 0) {
    ...
    for (const s of summaries.slice(0, 5)) {
      ...
      console.log(
        `    Days Active: ${s.daysActive} | Risk: ${s.currentRiskScore} | Promo: ${s.currentPromotionScore}`,
      );
    }
  }
}
