...
async function main() {
  ...
  console.log(
    "║     ScamDunk US Stock Evaluation - FINNHUB (60 req/min free tier)        ║",
  );
  console.log(
    "╚═══════════════════════════════════════════════════════════════════════════╝\n",
  );
  ...
  const checkpointPath = path.join(
    RESULTS_DIR,
    `checkpoint-finnhub-${timestamp}.json`,
  );
  ...
  if (startIndex > 0) {
    console.log(
      `📂 Resuming: ${results.length} done, ${skipped.length} skipped\n`,
    );
  }
  ...
  for (let i = startIndex; i < stocks.length; i++) {
    ...
    process.stdout.write(
      `\r${progressBar(progress, stocks.length)} ${progress}/${stocks.length} | ✓${successCount} ✗${skipped.length} | ETA: ${formatTime(eta)}  `,
    );
  ...
  const resultsPath = path.join(
    RESULTS_DIR,
    `evaluation-finnhub-${timestamp}.json`,
  );
  ...
  const summaryPath = path.join(
    RESULTS_DIR,
    `summary-finnhub-${timestamp}.json`,
  );
  const summary = {
  ...
  for (const r of results) {
    if (!summary.byExchange[r.exchange])
      summary.byExchange[r.exchange] = { total: 0, HIGH: 0 };
  ...
  const highCsv =
    "Symbol,Name,Exchange,MarketCap,Price,Score,Signals\n" +
    highRisk
      .map(
        (r) =>
          `"${r.symbol}","${r.name.replace(/"/g, '""')}","${r.exchange}",${r.marketCap || ""},${r.lastPrice || ""},${r.totalScore},"${r.signalSummary}"`,
      )
      .join("\n");
  fs.writeFileSync(
    path.join(RESULTS_DIR, `high-risk-finnhub-${timestamp}.csv`),
    highCsv,
  );
  ...
  const medCsv =
    ...
    medRisk
      ...
      .join("\n");
  fs.writeFileSync(
    path.join(RESULTS_DIR, `medium-risk-finnhub-${timestamp}.csv`),
    medCsv,
  );

  const lowRisk = results.filter((r) => r.riskLevel === "LOW");
  const lowCsv =
    "Symbol,Name,Exchange,MarketCap,Price,Score,IsLegitimate\n" +
    lowRisk
      .map(
        (r) =>
          `"${r.symbol}","${r.name.replace(/"/g, '""')}","${r.exchange}",${r.marketCap || ""},${r.lastPrice || ""},${r.totalScore},${r.isLegitimate}`,
      )
      .join("\n");
  fs.writeFileSync(
    path.join(RESULTS_DIR, `low-risk-finnhub-${timestamp}.csv`),
    lowCsv,
  );
  ...
  if (fs.existsSync(jan1Path)) {
    ...
    console.log(
      "                    COMPARISON: Jan 1st vs Today                    ",
    );
    console.log(
      "═══════════════════════════════════════════════════════════════════\n",
    );

    const jan1: EvaluationResult[] = JSON.parse(
      fs.readFileSync(jan1Path, "utf-8"),
    );
    const jan1Map = new Map(
      jan1
        .filter((r) => r.priceDataSource === "yahoo")
        .map((r) => [r.symbol, r]),
    );
      ...
      if (currRisk > prevRisk && prevRisk >= 0) {
        ...
        if (curr.riskLevel === "HIGH" && prev.riskLevel !== "HIGH") {
          newHighRisk.push({
            ...
            currentScore: curr.totalScore,
            changeType: "DOWNGRADED",
            previousSignals: prev.signalSummary,
            currentSignals: curr.signalSummary,
          });
        }
    ...
    console.log(
      `Today HIGH risk:   ${currHigh} (${currHigh > jan1High ? "+" : ""}${currHigh - jan1High})`,
    );
    ...
    if (newHighRisk.length > 0) {
      ...
      newHighRisk.slice(0, 20).forEach((s, i) => {
        console.log(
          `  ${i + 1}. ${s.symbol.padEnd(8)} ${s.previousRisk} → ${s.currentRisk} (${s.previousScore} → ${s.currentScore})`,
        );
      });
    }
