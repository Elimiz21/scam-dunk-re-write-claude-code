...
async function main() {
  ...
  console.log(
    "Uses actual Reddit, StockTwits, and YouTube APIs - NO AI predictions",
  );
  console.log("=".repeat(70));
  ...
  console.log(
    `  YouTube API: ${process.env.YOUTUBE_API_KEY ? "✓ Configured" : "✗ Not set (YouTube scans will be skipped)"}`,
  );
  ...
  for (let i = 0; i < stocks.length; i++) {
    ...
    try {
      ...
      // Summary
      const realMentions = result.platforms.reduce(
      ...
      console.log(
        `  Has promotional evidence: ${hasRealEvidence ? "YES" : "No"}`,
      );
      ...
      for (const platform of result.platforms) {
        if (platform.success) {
          console.log(
            `    ${platform.platform}: ${platform.mentionsFound} mentions (${platform.promotionRisk} risk)`,
          );
        } else {
          console.log(
            `    ${platform.platform}: ${platform.error || "Failed"}`,
          );
        }
      }
    } catch (error: any) {
  ...
  const totalMentions = results.reduce(
    (sum, r) => sum + r.platforms.reduce((s, p) => s + p.mentionsFound, 0),
    0,
  );
  ...
  if (withEvidence.length > 0) {
    ...
    for (const r of withEvidence) {
      const platforms = r.platforms
        ...
        .join(", ");
      console.log(
        `  ${r.symbol}: ${r.overallPromotionScore}/100 - ${platforms}`,
      );
    }
  }
  ...
  const summaryPath = path.join(
    RESULTS_DIR,
    `real-social-scan-summary-${date}.md`,
  );
