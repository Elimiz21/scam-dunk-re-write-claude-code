  ...
  try {
    ...
    return rows
      .map((row: any) => ({
        ...
        lastPrice:
          parseFloat(row.lastsale?.replace("$", "").replace(",", "")) ||
          undefined,
...
async function fetchAllUSStocks(): Promise<void> {
  ...
  const csvHeader =
    "Symbol,Name,Exchange,MarketCap,Sector,Industry,LastPrice,Volume,IsOTC\n";
  const csvRows = uniqueStocks
    .map(
      (s) =>
  ...
  // Summary by exchange
  const byExchange = uniqueStocks.reduce(
    (acc, s) => {
  ...
  const byMarketCap = {
    "Large Cap (>$10B)": uniqueStocks.filter(
      (s) => s.marketCap && s.marketCap > 10_000_000_000,
    ).length,
    "Mid Cap ($2B-$10B)": uniqueStocks.filter(
      (s) =>
        s.marketCap &&
        s.marketCap >= 2_000_000_000 &&
        s.marketCap <= 10_000_000_000,
    ).length,
    "Small Cap ($300M-$2B)": uniqueStocks.filter(
      (s) =>
        s.marketCap &&
        s.marketCap >= 300_000_000 &&
        s.marketCap < 2_000_000_000,
    ).length,
    "Micro Cap (<$300M)": uniqueStocks.filter(
      (s) => s.marketCap && s.marketCap < 300_000_000,
    ).length,
