...
async function fetchMarketData(ticker: string): Promise<MarketData> {
  ...
  const isOTC =
    quote.exchange.includes("OTC") || quote.exchange.includes("PINK");
...
async function main() {
  ...
  console.log(
    "║    ScamDunk US Stock Evaluation - FINNHUB (curl-based)                   ║",
  );
  console.log(
    "╚═══════════════════════════════════════════════════════════════════════════╝\n",
  );
  ...
  console.log(
    `⏱️  Delay: ${DELAY_MS}ms per request (${REQUESTS_PER_MINUTE}/min)\n`,
  );
  ...
  let success = checkpoint.results.filter(
    (r) => r.riskLevel !== "INSUFFICIENT",
  ).length;
  let failed = checkpoint.results.filter(
    (r) => r.riskLevel === "INSUFFICIENT",
  ).length;
...
function generateSummary(results: StockResult[]) {
  ...
  console.log(
    `  LOW:    ${distribution.LOW} (${((distribution.LOW / validResults.length) * 100).toFixed(1)}%)`,
  );
  console.log(
    `  MEDIUM: ${distribution.MEDIUM} (${((distribution.MEDIUM / validResults.length) * 100).toFixed(1)}%)`,
  );
  console.log(
    `  HIGH:   ${distribution.HIGH} (${((distribution.HIGH / validResults.length) * 100).toFixed(1)}%)`,
  );
