    ...
    return stocks.map((s: any) => ({
      ...
      signals:
        s.signals?.map((sig: any) =>
          typeof sig === "string" ? sig : sig.code,
        ) || [],
    }));
  }
...
function aggregateResults(
  ...
  for (const target of targets) {
    ...
    const platformsWithMentions = Array.from(
      new Set(uniqueMentions.map((m) => m.platform)),
    );
    ...
    const highRiskPlatforms = platformsWithMentions.filter((p) => {
      ...
      const avgScore =
        platformMentions.reduce((s, m) => s + m.promotionScore, 0) /
        platformMentions.length;
      return avgScore >= 40;
    });
    ...
    const riskLevel =
      highRiskPlatforms.length >= 2
        ? "high"
        : highRiskPlatforms.length >= 1 || overallPromotionScore >= 50
          ? "medium"
          : "low";
...
export async function runSocialScan(options: {
  ...
  targets.forEach((t) =>
    console.log(
      `  ${t.ticker} (${t.name}) - Risk: ${t.riskLevel}, Score: ${t.riskScore}`,
    ),
  );
  ...
  for (const scanner of scanners) {
    ...
    try {
      ...
      for (const r of results) {
        if (r.success) {
          console.log(
            `  ✓ ${r.platform}: ${r.mentionsFound} mentions found (${r.activityLevel} activity, ${r.promotionRisk} promotion risk)`,
          );
        } else {
  ...
  if (tickerResults.filter((r) => r.riskLevel === "high").length > 0) {
    ...
    for (const r of tickerResults.filter((r) => r.riskLevel === "high")) {
      console.log(
        `  ${r.ticker} (${r.name}): ${r.totalMentions} mentions, score ${r.overallPromotionScore}/100`,
      );
      console.log(`    ${r.summary}`);
      if (r.topPromoters.length > 0) {
        console.log(
          `    Top promoters: ${r.topPromoters.map((p) => `${p.username}@${p.platform}`).join(", ")}`,
        );
      }
    }
  }
