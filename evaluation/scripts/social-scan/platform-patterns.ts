...
export const STOCKTWITS_REGEX_PATTERNS: Array<{
  regex: RegExp;
  flag: string;
  score: number;
}> = [
...
export const YOUTUBE_REGEX_PATTERNS: Array<{
  regex: RegExp;
  flag: string;
  score: number;
}> = [
  ...
  {
    regex:
      /(?:webull|moomoo|robinhood|coinbase)\.com\/\S*(?:ref|invite|promo)/gi,
...
export const REDDIT_REGEX_PATTERNS: Array<{
  regex: RegExp;
  flag: string;
  score: number;
}> = [
...
export const DISCORD_TELEGRAM_REGEX_PATTERNS: Array<{
  regex: RegExp;
  flag: string;
  score: number;
}> = [
...
export const TWITTER_REGEX_PATTERNS: Array<{
  regex: RegExp;
  flag: string;
  score: number;
}> = [
  ...
  {
    regex:
      /\$[A-Z]{2,5}\s+\$\d+\.?\d*\s+by\s+(?:Q[1-4]|eoy|end\s+of\s+year|summer|fall|spring|winter|\d{4})/gi,
  ...
  {
    regex:
      /(?:called|alerted|bought)\s+\$[A-Z]{2,5}\s+(?:at|@)\s*\$?\d+\.?\d*.*(?:now|currently)\s+\$?\d+\.?\d*/gi,
...
export const TIKTOK_REGEX_PATTERNS: Array<{
  regex: RegExp;
  flag: string;
  score: number;
}> = [
 ...
 */
export function getPatternCounts(): Record<
  PlatformName,
  { high: number; medium: number; low: number; regex: number; total: number }
> {
  const result: Record<
    string,
    { high: number; medium: number; low: number; regex: number; total: number }
  ...
  for (const [name, patterns] of Object.entries(PLATFORM_PATTERNS)) {
    result[name] = {
      ...
      total:
        patterns.high.length +
        patterns.medium.length +
        patterns.low.length +
