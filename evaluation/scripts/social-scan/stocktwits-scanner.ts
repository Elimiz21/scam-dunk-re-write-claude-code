  ...
  async scan(targets: ScanTarget[]): Promise<PlatformScanResult[]> {
    ...
    for (const target of targets) {
      try {
        ...
        if (!response.ok) {
          console.error(
            `    StockTwits ${response.status} for ${target.ticker}`,
          );
          hasErrors = true;
          continue;
        }
        ...
        if (text.startsWith("<")) {
          console.error(
            `    StockTwits returned HTML for ${target.ticker} (likely blocked)`,
          );
          hasErrors = true;
          continue;
        }
    ...
    return [
      {
        ...
        error: hasErrors
          ? "Some tickers failed (possibly rate limited or blocked)"
          : undefined,
