...
export function saveSchemeDatabase(db: SchemeDatabase): void {
  ...
  db.activeSchemes = schemes.filter((s) =>
    ["NEW", "ONGOING"].includes(s.status),
  ).length;
  db.resolvedSchemes = schemes.filter((s) => s.status === "RESOLVED").length;
  db.confirmedFrauds = schemes.filter(
    (s) => s.status === "CONFIRMED_FRAUD",
  ).length;
...
export function updateSchemeRecord(
  ...
  const daysActive =
    Math.ceil(
      (todayDate.getTime() - firstDetectedDate.getTime()) /
        (1000 * 60 * 60 * 24),
    ) + 1;
  ...
  if (data.price > scheme.peakPrice) {
    const priceIncrease = (
      ((data.price - scheme.peakPrice) / scheme.peakPrice) *
      100
    ).toFixed(1);
  ...
  scheme.priceChangeFromDetection =
    ((data.price - scheme.priceAtDetection) / scheme.priceAtDetection) * 100;
  scheme.priceChangeFromPeak =
    ((data.price - scheme.peakPrice) / scheme.peakPrice) * 100;
    ...
    for (const account of data.newPromoterAccounts) {
      const existing = scheme.promoterAccounts.find(
        (p) =>
          ...
          p.identifier === account.identifier,
      );
...
export function getSchemeUrgency(
  scheme: SchemeRecord,
...
export function getSchemeSummaries(db: SchemeDatabase): SchemeSummary[] {
  return Object.values(db.schemes)
    .filter((s) =>
      ["NEW", "ONGOING", "COOLING", "CONFIRMED_FRAUD"].includes(s.status),
    )
    .map((s) => ({
      schemeId: s.schemeId,
      symbol: s.symbol,
...
export function generateDailySchemeReport(
  db: SchemeDatabase,
  date: string,
): string {
...
export function exportSchemeData(
  db: SchemeDatabase,
...
export function archiveOldSchemes(
  db: SchemeDatabase,
  daysOld: number = 30,
): { archived: number; remaining: number } {
  ...
  switch (command) {
    case "list":
      ...
      for (const s of summaries) {
        ...
        console.log(
          `  Risk: ${s.currentRiskScore} | Promo: ${s.currentPromotionScore}`,
        );
        console.log(
          `  Price: ${s.priceChangeFromDetection.toFixed(1)}% from detection`,
        );
        console.log("");
      }
      break;
    ...
    case "archive":
      ...
      console.log(
        `Archived ${result.archived} schemes older than ${days} days`,
      );
