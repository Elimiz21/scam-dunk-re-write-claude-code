...
function getConfidenceLevel(stock: EvaluationResult): string {
  ...
  if (
    hasPatternSignals >= 2 ||
    (hasPatternSignals >= 1 && hasStructuralSignals >= 2)
  )
    return "HIGH";
...
function analyze() {
  ...
  const bothSignals = results.filter(
    (r) =>
      r.signals.some((s) => s.code === "SPIKE_THEN_DROP") &&
      r.signals.some((s) => s.code === "VOLUME_EXPLOSION"),
  );
  ...
  console.log(
    "║           DEEP ANALYSIS: Pump-and-Dump & Volume Explosion Stocks          ║",
  );
  console.log(
    "╚════════════════════════════════════════════════════════════════════════════╝\n",
  );
  ...
  console.log("\n" + "=".repeat(80));
  console.log(
    "SECTION 1: LARGE CAP STOCKS (Market Cap > $1B) - LIKELY LEGITIMATE",
  );
  console.log("=".repeat(80));
  console.log(
    "These are established companies. Their patterns are likely due to:",
  );
  ...
  const largeCap = pumpAndDump.filter(
    (s) => s.marketCap && s.marketCap >= LARGE_CAP_THRESHOLD,
  );
  ...
  largeCap.forEach((stock, i) => {
    const mcStr = stock.marketCap
      ...
      : "N/A";
    const volatileSector = isVolatileSector(stock.name)
      ? " [VOLATILE SECTOR]"
      : "";
    console.log(
      `${(i + 1).toString().padStart(2)}. ${stock.symbol.padEnd(6)} | ${mcStr.padStart(10)} | ${stock.name.substring(0, 40)}${volatileSector}`,
    );
    console.log(`    Signals: ${stock.signalSummary}`);
    console.log(
      `    Price: $${stock.lastPrice?.toFixed(2)} | Score: ${stock.totalScore}`,
    );
    console.log("");
  });
  ...
  console.log("=".repeat(80));
  console.log(
    "These require individual investigation. Some may be legitimate volatility,",
  );
  console.log("others could be manipulation targets.");
  console.log("-".repeat(80));

  const midCap = pumpAndDump.filter(
    (s) =>
      s.marketCap &&
      s.marketCap >= MID_CAP_THRESHOLD &&
      s.marketCap < LARGE_CAP_THRESHOLD,
  );
  ...
  midCap.forEach((stock, i) => {
    const mcStr = stock.marketCap
      ? `$${(stock.marketCap / 1_000_000).toFixed(0)}M`
      : "N/A";
    ...
    const volatileSector = isVolatileSector(stock.name)
      ...
      : "";
    console.log(
      `${(i + 1).toString().padStart(2)}. ${stock.symbol.padEnd(6)} | ${mcStr.padStart(8)} | Confidence: ${confidence.padEnd(9)} | ${stock.name.substring(0, 35)}${volatileSector}`,
    );
  });
  ...
  bothSignals.slice(0, 30).forEach((stock, i) => {
    ...
    console.log(
      `${(i + 1).toString().padStart(2)}. ${stock.symbol.padEnd(6)} | Score: ${stock.totalScore.toString().padStart(2)} | ${mcStr.padStart(8)} | ${capCategory}`,
    );
  ...
  console.log("\n" + "=".repeat(80));
  console.log(
    "SECTION 4: MICRO CAP STOCKS (<$50M) - HIGHEST MANIPULATION RISK",
  );
  console.log("=".repeat(80));
  console.log("These are prime pump-and-dump targets due to low liquidity.");
  console.log("-".repeat(80));

  const microCap = pumpAndDump.filter(
    (s) => !s.marketCap || s.marketCap < MICRO_CAP_THRESHOLD,
  );
  ...
  microCap.slice(0, 50).forEach((stock, i) => {
    ...
    const confidence = getConfidenceLevel(stock);
    console.log(
      `${(i + 1).toString().padStart(2)}. ${stock.symbol.padEnd(6)} | Score: ${stock.totalScore.toString().padStart(2)} | ${mcStr.padStart(8)} | Conf: ${confidence.padEnd(9)} | $${stock.lastPrice?.toFixed(2)}`,
    );
  ...
  console.log("\n" + "=".repeat(80));
  console.log(
    "SECTION 5: VOLUME EXPLOSION ONLY (No Pump-and-Dump Pattern Yet)",
  );
  console.log("=".repeat(80));
  console.log(
    'These stocks have unusual volume but have NOT yet shown the "dump" phase.',
  );
  console.log("They could be in the EARLY STAGES of a pump scheme.");
  console.log("-".repeat(80));

  const volumeOnly = volumeExplosion.filter(
    (s) => !s.signals.some((sig) => sig.code === "SPIKE_THEN_DROP"),
  );
  ...
  const capDistribution = {
    ...
    "Small Cap ($50M-$300M)": pumpAndDump.filter(
      (s) =>
        s.marketCap &&
        s.marketCap >= MICRO_CAP_THRESHOLD &&
        s.marketCap < MID_CAP_THRESHOLD,
    ).length,
  ...
  Object.entries(capDistribution).forEach(([cap, count]) => {
    ...
    console.log(
      `  ${cap.padEnd(25)}: ${count.toString().padStart(3)} (${pct}%)`,
    );
  });

  // Confidence level distribution
  const confidenceDistribution = {
    "VERY HIGH": pumpAndDump.filter(
      (s) => getConfidenceLevel(s) === "VERY HIGH",
    ).length,
    HIGH: pumpAndDump.filter((s) => getConfidenceLevel(s) === "HIGH").length,
    MEDIUM: pumpAndDump.filter((s) => getConfidenceLevel(s) === "MEDIUM")
      .length,
  ...
  Object.entries(confidenceDistribution).forEach(([level, count]) => {
    const pct = ((count / pumpAndDump.length) * 100).toFixed(1);
    console.log(
      `  ${level.padEnd(10)}: ${count.toString().padStart(3)} (${pct}%)`,
    );
  });

  // Volatile sector analysis
  const volatileSectorCount = pumpAndDump.filter((s) =>
    isVolatileSector(s.name),
  ).length;
  console.log(
  ...
  console.log(
    `3. HIGH CONFIDENCE manipulation candidates: ${trulyHighRisk.length} stocks`,
  );
  ...
  const revisedHighConfidence =
    trulyHighRisk.length +
    bothSignals.filter((s) => !s.marketCap || s.marketCap < LARGE_CAP_THRESHOLD)
      .length;

  console.log("REVISED STATISTICS FOR PRESS:");
  console.log(
    `  - Total pump-and-dump patterns detected: ${pumpAndDump.length}`,
  );
  console.log(`  - Likely legitimate (large cap): ${largeCap.length}`);
  console.log(`  - High-confidence manipulation: ${trulyHighRisk.length}`);
  console.log(
    `  - Volume explosions (potential early-stage): ${volumeOnly.length}`,
  );
  console.log(`  - Stocks with BOTH signals: ${bothSignals.length}`);
}
