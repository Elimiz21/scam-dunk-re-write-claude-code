...
async function runRegularStocksScan(): Promise<void> {
  const evaluationDate =
    process.env.EVALUATION_DATE || new Date().toISOString().split("T")[0];
  ...
  const regularStocks = highRiskStocks.filter(
    (s) => !suffixesToExclude.some((suffix) => s.symbol.endsWith(suffix)),
  );

  console.log(`Total high-risk stocks: ${highRiskStocks.length}`);
  console.log(
    `Regular stocks (excluding warrants/rights): ${regularStocks.length}\n`,
  );
  ...
  const maxStocks = process.env.SOCIAL_SCAN_LIMIT
    ...
    : regularStocks.length;
  const stocksToScan = regularStocks
    .sort((a, b) => b.totalScore - a.totalScore)
    .slice(0, maxStocks);

  console.log(
    `Scanning top ${stocksToScan.length} regular high-risk stocks:\n`,
  );
  ...
  for (let i = 0; i < stocksToScan.length; i++) {
    ...
    try {
      ...
      if (scanResult) {
        ...
        if (scanResult.overallPromotionScore >= 30) {
          const activePlatforms = scanResult.platforms
            .filter(
              (p) => p.promotionRisk === "high" || p.promotionRisk === "medium",
            )
            .map((p) => p.platform);
          ...
          console.log(
            `  🔴 PROMOTION DETECTED: Score ${scanResult.overallPromotionScore}/100`,
          );
          console.log(
            `     Platforms: ${activePlatforms.join(", ") || "General activity"}`,
          );
          ...
          console.log(
            `  🟡 MODERATE activity: Score ${scanResult.overallPromotionScore}/100`,
          );
        } else {
          console.log(
            `  🟢 LOW activity: Score ${scanResult.overallPromotionScore}/100`,
          );
        }

        // Show mention counts
        const totalMentions = scanResult.platforms.reduce(
      ...
      console.log(
        `  ⚠️ Error scanning ${stock.symbol}: ${error?.message || error}`,
      );
    }
  }
  ...
  const outputPath = path.join(
    RESULTS_DIR,
    `social-media-scan-regular-${evaluationDate}.json`,
  );
  ...
  if (promotedStocks.length > 0) {
    ...
    promotedStocks
      ...
      .forEach((s, i) => {
        ...
        console.log(
          `   Risk Score: ${s.riskScore} | Promotion Score: ${s.promotionScore}/100`,
        );
        console.log(
          `   Active Platforms: ${s.platforms.join(", ") || "General activity"}`,
        );
    ...
    console.log(
      "  - Promotion happening on platforms we cannot scan (Discord, Telegram)",
    );
    console.log(
      "  - Price movements driven by other factors (news, market conditions)",
    );
...
`;
  ...
  const reportPath = path.join(
    RESULTS_DIR,
    `social-media-scan-regular-${evaluationDate}.md`,
  );
