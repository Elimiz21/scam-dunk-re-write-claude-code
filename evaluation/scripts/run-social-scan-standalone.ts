    ...
    try {
      ...
      if (scanResult) {
        ...
        if (scanResult.overallPromotionScore >= 40) {
          ...
          console.log(
            `  🔴 HIGH PROMOTION DETECTED: Score ${scanResult.overallPromotionScore}/100`,
          );
          console.log(
            `     Platforms: ${activePlatforms.join(", ") || "None"}`,
          );
...
${
  promotedStocks.length === 0
    ? "No significant promotion activity detected."
    : promotedStocks
        .map(
          (s, i) => `
### ${i + 1}. ${s.symbol} - ${s.name}
...
`;
  ...
  const reportPath = path.join(
    RESULTS_DIR,
    `social-media-scan-${evaluationDate}.md`,
  );
