...
function fetchGitHubContents<T>(path: string): T | null {
  try {
    ...
    const result = execSync(`curl -s --max-time 30 "${apiUrl}"`, {
      encoding: "utf-8",
      maxBuffer: 100 * 1024 * 1024,
    });
    const apiResponse = JSON.parse(result);
    if (apiResponse.content) {
      const decoded = Buffer.from(apiResponse.content, "base64").toString(
        "utf-8",
      );
...
async function main() {
  ...
  for (const [, scheme] of Object.entries(schemes)) {
    ...
    if (
      ...
      typeof scheme.promoterAccounts[0] === "string"
    ) {
    ...
    console.log(
      `   ${scheme.symbol}: ${scheme.promoterAccounts.length} promoter accounts across ${platforms.length} platforms`,
    );
  }
  ...
  for (const scheme of schemeValues) {
    ...
    for (const account of scheme.promoterAccounts) {
      ...
      if (!promoter) {
        const idPart = account.identifier
          .replace(/[^a-zA-Z0-9]/g, "")
          .toUpperCase()
          .slice(0, 10);
        const platPart = account.platform
          .replace(/[^a-zA-Z]/g, "")
          .toUpperCase()
          .slice(0, 6);
        promoter = {
  ...
  console.log("=".repeat(70));
  console.log(
    `  Schemes: ${schemeValues.length} total (${updatedSchemeDb.activeSchemes} active)`,
  );
  console.log(
    `  Promoters: ${promoterList.length} total (${promoterDb.activePromoters} active)`,
  );
  console.log(`  Serial offenders: ${promoterDb.serialOffenders}`);
  console.log(
    `  Co-promoter pairs: ${promoterList.reduce((sum, p) => sum + p.coPromoters.length, 0) / 2}`,
  );
  console.log("");
  ...
  console.log(
    "  3. Upload to Supabase: npx ts-node scripts/upload-to-supabase.ts",
  );
}
