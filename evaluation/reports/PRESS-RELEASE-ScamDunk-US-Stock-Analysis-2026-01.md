# ScamDunk AI Analysis: 1,096 US Stocks Show High-Risk Manipulation Patterns
...
An independent analysis of 6,290 US-listed stocks using ScamDunk's AI-powered risk detection system has identified **1,096 stocks (17.4%)** exhibiting high-risk characteristics consistent with potential market manipulation, including classic "pump-and-dump" patterns.

The analysis detected **183 stocks actively showing pump-and-dump price patterns** and **90 stocks with suspicious volume explosions** — warning signs that retail investors should be aware of before investing.
...
## Highest Risk Stocks Identified
...
ScamDunk employs a multi-factor risk scoring system:

1. **Structural Risk Analysis**
   - Market capitalization thresholds (under $300M = elevated risk)
   - Stock price analysis (under $5 = penny stock territory)
   - Liquidity assessment (daily dollar volume)
   - Exchange type (OTC markets = highest structural risk)

2. **Pattern Recognition**
   - **Pump-and-Dump Detection:** Identifies stocks that spiked 25%+ followed by 20%+ decline within 15 days
   - **Volume Anomaly Detection:** Flags trading volume exceeding 3x the 30-day average
   - **Price Spike Analysis:** Detects abnormal 7-day price movements (25%+ or 50%+)
   - **RSI Overbought Conditions:** Technical analysis indicator above 70

3. **Risk Scoring**
   - Each signal carries a weight (1-4 points)
   - **HIGH Risk:** Score ≥ 5
...
**For data requests:**
Full dataset available upon request for qualified financial journalists and researchers.

**Website:**
[ScamDunk website URL]

---

## Appendix: Detailed Statistics

### Risk Signal Occurrence Rates

```
Signal                      Count    % of Stocks
─────────────────────────────────────────────────
SMALL_MARKET_CAP           2,019       32.1%
MICROCAP_PRICE             1,709       27.2%
HIGH_VOLATILITY              954       15.2%
MICRO_LIQUIDITY              883       14.0%
OVERBOUGHT_RSI               374        5.9%
SPIKE_7D                     239        3.8%
SPIKE_THEN_DROP              183        2.9%
VOLUME_EXPLOSION              90        1.4%
```

### Exchange Breakdown

```
Exchange    Total     LOW    MEDIUM    HIGH    % HIGH
────────────────────────────────────────────────────────
NASDAQ      3,677   1,595    1,129      953     25.9%
NYSE        2,341   1,968      333       40      1.7%
AMEX          272      61      108      103     37.9%
```

### Score Distribution (HIGH Risk Stocks Only)

```
Score Range    Count    Notable Examples
──────────────────────────────────────────
15+               2     ZCMD, VSSYW
14               20+    HXHX, PCLA, HIHO, XTKG, RPGL, SOBR, ECDA, GVH
12-13           100+    Various micro-cap stocks
10-11           200+    Small-cap with multiple signals
5-9             700+    Single or double signal triggers
```
